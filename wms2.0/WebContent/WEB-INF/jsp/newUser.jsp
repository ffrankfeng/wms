...
<jsp:include page="header.jsp"></jsp:include>
	...
	<form action="${pageContext.request.contextPath }/user/saveNewUser" method="post" onsubmit="return checksub()">
	<table class="table">
		<tr><td>姓名<input class="input3" id="user_name" name="user_name" ></td>
		<td>联系方式<input class="input4"id="user_phone" name="user_phone" onkeyup="value=value.replace(/[^\d]/g,'')"></td></tr>
		<tr>
		<td>密码<input class="input4" id="user_password" name="user_password" ></td>
...
function checksub(){
	var usernameInput=$("#user_name").val();
	var userpasswordInput=$("#user_password").val();
	var userphoneInput=$("#user_phone").val();
	var userautInput=$("#user_aut").val();
	...
	if(userpasswordInput.length<6||userpasswordInput.length>12){
		userpasswordInput="请设置6~12位密码";
		alert(userpasswordInput);
		return false;
	}
	if(userphoneInput.length!=11){
		userphoneInput="电话号码格式不正确";
		alert(userphoneInput);
		return false;
	}
	if(userautInput==""){
		userautInput="管理员权限不能为空";
		alert(userautInput);
		return false;
	}
	return true;
